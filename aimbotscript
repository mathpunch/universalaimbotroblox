...
-- UI Blur (Performance consideration for mobile)
...
-- Mobile/PC Input Handling for Dragging
local dragging, dragInput, dragStart, startPos

-- Dragging UI Element (Touch and Mouse Support)
...
-- Create a modern button with touch support
local function createModernButton(name, defaultText, callback)
    ...
    -- Shadow (optional, can be removed for performance on mobile)
    local shadow = Instance.new("UIStroke", button)
    shadow.Thickness = 1.5
    shadow.Color = Color3.fromRGB(0, 0, 0)
    shadow.Transparency = 0.5

    -- Hover effect (PC) and Tap effect (Mobile)
...
-- Mobile/PC UI Scaling (optional)
local isMobile = UserInputService.TouchEnabled
if isMobile then
    -- Adjust UI size for mobile screens if needed
